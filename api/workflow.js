...
export default async function handler(req,res){
  ...
  try{
    ...
    if(req.method==="POST"&&action==="match"){
      ...
      const accounts=await rest("/rest/v1/profiles?role=eq.recycler&active=eq.true&select=id,phone,name,business_name,general_location,accepted_materials,pickup_radius_km,latitude,longitude,verification_status,verification_badge,verified_at").catch(()=>[]);
      const accountRows=accounts.map(x=>({external_id:"ACCOUNT:"+x.phone,facility_name:x.business_name||x.name||"Recycler account",address:x.general_location||"",city:x.general_location||"",district:"",state:"Andhra Pradesh",materials_accepted:Array.isArray(x.accepted_materials)?x.accepted_materials:[],authorization_status:"Account registered",authorization_source:"Self-registered in Kabadiwala Connect",installed_capacity_mta:null,offered_rate_tier:"User-set offer",pickup_available:true,service_area_km:Number(x.pickup_radius_km)||5,latitude:x.latitude??null,longitude:x.longitude??null,verification_status:x.verification_status||"pending",verification_badge:!!x.verification_badge,verified_at:x.verified_at||null,account:true}));
