export default async function handler(req,res){
  ...
  try{
    ...
    const pQs=new URLSearchParams({select:"id,phone,name,business_name,general_location,accepted_materials,pickup_radius_km,latitude,longitude,active,verification_status,verification_badge,verified_at",role:"eq.recycler",active:"eq.true",order:"business_name.asc,name.asc"});
    ...
    const accounts=(Array.isArray(profiles)?profiles:[]).map(x=>({
      ...
      pickup_available:x.pickup_available!==false,service_area_km:Number(x.pickup_radius_km)||5,latitude:x.latitude??null,longitude:x.longitude??null,verification_status:x.verification_status||"pending",verification_badge:!!x.verification_badge,verified_at:x.verified_at||null,account:true
    }));
