...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function roleScreen(){
    A.innerHTML='<main class="auth"><div class="auth-card role-card"><button class="back" id="roleChangeNumber">← '+tr("change")+'</button><p class="eyebrow">ONE CHOICE</p><h1>'+tr("chooseRole")+'</h1><div class="role-grid"><button type="button" class="role-option" id="collectorCreate" data-role="collector" aria-label="Create Collector account"><span class="role-icon">♻</span><strong>Create Collector account</strong><small>'+tr("collectorHint")+'</small></button><button type="button" class="role-option" data-role="recycler"><span class="role-icon">⌂</span><strong>'+tr("recycler")+'</strong><small>'+tr("recyclerHint")+'</small></button></div><button class="secondary full" id="adminEntry" style="margin-top:14px">🛡 Admin panel</button><p id="adminEntryMsg" class="muted" style="margin-top:8px"></p></div></main>';
  ...
  function setupScreen(){
    ...
    const isEditingProfile=!!profileEditSnapshot;
    ...
    A.innerHTML='<main class="auth"><div class="auth-card setup-card wide-setup"><button type="button" class="back" id="setupBack">← '+(isEditingProfile?tr("profile"):tr("change"))+'</button><p class="eyebrow">'+tr("setup")+'</p><h1>'+tr(role)+'</h1><form id="setupForm"><label>'+tr("name")+'<input id="name" value="'+esc(profile?.name||"")+'" required></label>'+
    ...
    document.getElementById("setupForm").onsubmit=async e=>{
      ...
      await syncPending();profileEditSnapshot=null;go("dashboard");
    };
  }
...
function listScreen(){
  ...
  function profileScreen(){
    A.innerHTML=topbar()+'<main class="page narrow"><section class="section-title"><div><button class="back" id="profileBack">← '+tr("dashboard")+'</button><p class="eyebrow">'+tr("profile")+'</p><h1>'+esc(profile?.name||"")+'</h1></div></section><section class="panel profile-panel"><div class="profile-row"><span>'+tr("phone")+'</span><b>+91 '+esc(user?.phone||"")+'</b></div><div class="profile-row"><span>'+tr("role")+'</span><b>'+tr(role)+'</b></div><div class="profile-row"><span>'+tr("area")+'</span><b>'+esc(profile?.area||"")+'</b></div><div class="profile-row"><span>'+tr("language")+'</span><select id="profileLang"><option value="en">English</option><option value="hi">हिन्दी</option><option value="mr">मराठी</option></select></div><button class="secondary full" id="edit">'+tr("edit")+'</button><button class="danger full" id="out">'+tr("signOut")+'</button></section></main>';
    bindShell();document.getElementById("profileBack").onclick=()=>go("dashboard");document.getElementById("profileLang").value=lang;document.getElementById("profileLang").onchange=e=>{lang=e.target.value;save();render();};document.getElementById("edit").onclick=()=>{profileEditSnapshot={profile:JSON.parse(JSON.stringify(profile||{})),pos:pos?{...pos}:null};go("setup");};document.getElementById("out").onclick=async()=>{if(confirm(tr("confirmReset"))){await fetch("/api/session",{method:"DELETE",credentials:"same-origin"}).catch(()=>{});sessionReady=false;localStorage.clear();location.hash="login";render();}};
  }
  ...
  async function adminScreen(){
    ...
    A.innerHTML=topbar()+'<main class="page admin-page"><section class="section-title"><div><p class="eyebrow">🛡 '+tr("adminPanel")+'</p><h1>'+tr("summary")+'</h1><p>Full platform control: recycler verification, users, requests, transactions, earnings and the official market price board.</p></div><button class="secondary" id="adminRefresh">↻ '+tr("refresh")+'</button></section>'+(error?'<div class="panel error-panel">'+esc(error)+'</div>':'')+
      ...
      '<section class="admin-data"><div class="section-title admin-section-title"><div><h1>👥 User management</h1><p>View all collector and recycler accounts, deactivate/delete accounts, and restore deactivated accounts.</p></div></div><div class="panel"><div class="admin-table-wrap"><table class="admin-table"><thead><tr><th>Role</th><th>Phone</th><th>Name / Business</th><th>Status</th><th>Verification</th><th>Action</th></tr></thead><tbody>'+((data.all_collectors&&data.all_collectors.length?data.all_collectors:(data.collectors||[])).concat(data.all_recyclers&&data.all_recyclers.length?data.all_recyclers:(data.recyclers||[])).map(x=>{const isR=String(x.role||"")==="recycler"||("business_name" in x);const active=x.active!==false;return '<tr><td>'+esc(isR?"Recycler":"Collector")+'</td><td>'+esc(x.phone||"—")+'</td><td>'+esc(x.business_name||x.name||"—")+'</td><td>'+esc(active?"Active":"Deleted")+'</td><td>'+esc(isR?(x.verification_status||"pending"):"—")+'</td><td>'+(active?'<button class="secondary admin-user-delete" data-phone="'+esc(x.phone)+'">Delete / Deactivate</button>':'<button class="primary admin-user-restore" data-phone="'+esc(x.phone)+'">Restore</button>')+'</td></tr>'}).join("")||'<tr><td colspan="6">No users.</td></tr>')+'</tbody></table></div></div></section>'+
    ...
    A.querySelectorAll(".admin-user-delete").forEach(b=>b.onclick=async()=>{if(!confirm("Delete this user account?"))return;try{const r=await fetch("/api/admin?action=delete-user",{method:"POST",headers:{"Content-Type":"application/json"},credentials:"same-origin",body:JSON.stringify({phone:b.dataset.phone})});const j=await r.json().catch(()=>({}));if(!r.ok)throw new Error(j.error||j.detail||"Could not delete user.");await adminScreen();}catch(e){toast(e.message||"Could not delete user.");}});
